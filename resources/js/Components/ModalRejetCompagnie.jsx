...
export default function ModalRejetCompagnie({ show, onClose,selecteCompagnie }) {
    const {
        delete: destroy,
        processing,
        reset,
    ...
    const deleteCompagnie = (e) => {
        e.preventDefault();

        destroy(route('compagnie.destroy', { compagnie: selecteCompagnie }), {
            preserveScroll: true,
            onSuccess: () => {
                reset();
                onClose();
    ...
    return (
        <Modal show={show} onClose={onClose}>
            <form  onSubmit={deleteCompagnie} className="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                <button
                    type="button"
